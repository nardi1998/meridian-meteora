...
import { getLastBriefingDate, setLastBriefingDate, getTrackedPosition, getTrackedPositions, setPositionInstruction, updatePnlAndCheckExits, queuePeakConfirmation, resolvePendingPeak, queueTrailingDropConfirmation, resolvePendingTrailingDrop, save, loadTrackedWithSave, registerExitSignal, confirmPeak } from "./state.js";
...
export async function runManagementCycle({ silent = false } = {}) {
  ...
  try {
    ...
    // ── Execute actions (CLOSE/CLAIM directly, INSTRUCTION via LLM) ──
    ...
    if (actionPositions.length > 0) {
      const resultText = await executeManagementActions(actionPositions, actionMap, { liveMessage, cur });
      if (resultText) mgmtReport += `\n\n${resultText}`;
    } else {
...
export async function runScreeningCycle({ silent = false } = {}) {
  ...
  try {
    ...
    for (const pool of candidates) {
      ...
      const [smartWallets, narrative, tokenInfo, gmgnData] = await Promise.allSettled([
      ...
      allCandidates.push({
        ...
        gmgnData: gmgnData.status === "fulfilled" ? gmgnData.value : null,
    ...
    const filteredOut = [];
    const passing = allCandidates.filter(({ pool, ti, gmgnData }) => {
      ...
      const globalFeesSol = Number(gmgnData?.fees ?? pool.gmgn_total_fee_sol ?? ti?.global_fes_sol);
...
3. If a pool qualifies, call deploy_position (active_bin is pre-fetched above — no need to call get_active_bin).
   ...
   bins_below = round(${config.strategy.minBinsBelow} + sqrt(candidate volatility) * 30) clamped to [${config.strategy.minBinsBelow},${config.strategy.maxBinsBelow}].
...
function getDeterministicCloseRule(position, managementConfig) {
  ...
  if (!pnlSuspect && position.pnl_pct != null && position.pnl_pct <= managementConfig.stopLossPct && softRuleAgeGuard) {
    return { action: "CLOSE", rule: 1, reason: "stop loss" };
  }
  if (!pnlSuspect && position.pnl_pct != null && position.pnl_pct >= managementConfig.takeProfitPct && softRuleAgeGuard) {
  ...
  if (
    ...
    // Close when PnL dips below peak by certain amount
    else if (peak < 1) {
      // Never touched +1% in 2h → close
      return { action: "CLOSE", rule: 8, reason: `failed target: peak ${peak.toFixed(2)}% < 1%` };
    } else if (peak < 1.5) {
      // Never touched +1.5% in 2h → close
      return { action: "CLOSE", rule: 8, reason: `failed target: peak ${peak.toFixed(2)}% < 1.5%` };
    } else if (peak < 2) {
      // Never touched +2% in 2h → close
      return { action: "CLOSE", rule: 8, reason: `failed target: peak ${peak.toFixed(2)}% < 2%` };
    }
  }
...
function computeBinsBelow(volatility) {
  ...
  // Square root scale: bins_below = 35 + sqrt(volatility) * 30
  return Math.max(lo, Math.min(hi, Math.round(lo + Math.sqrt(parsedVolatility) * 30)));
}
